...
export function CADAgentWorkbench({
  ...
  const persistentAttachment = trpc.persistentMemory.attach.useMutation();
  ...
  const pickFiles = async () => {
    try {
      ...
      for (const asset of result.assets) {
        if (!persistentProject || !activeConversationId) { setWorkbenchNotice("Persistent project memory is initializing. The file was not associated or interpreted."); return; }
        persistentAttachment.mutate({ projectId: persistentProject.projectId, accessKey: persistentProject.accessKey, conversationId: activeConversationId, name: asset.name, sizeBytes: asset.size, mimeType: asset.mimeType }, { onSuccess: (attachment) => { setAttachments((items) => [...items, attachment]); void memorySnapshot.refetch(); }, onError: (error) => setWorkbenchNotice(`Attachment metadata could not be associated: ${error.message}`) });
      }
    } catch (error) {
  ...
  const newConversation = () => {
    if (!persistentProject) return;
    createPersistentConversation.mutate({ projectId: persistentProject.projectId, accessKey: persistentProject.accessKey, title: "New CAD Agent conversation", reason: "User opened a new conversation branch" }, { onSuccess: (conversation) => { setActiveConversationId(conversation.id); setConversationTitleDraft(conversation.title); setMessages([]); setProposals([]); setConcepts([]); setEvidence([]); setHistory([]); setAttachments([]); void conversations.refetch(); void memorySnapshot.refetch(); }, onError: (error) => setWorkbenchNotice(`Conversation could not be created: ${error.message}`) });
  };
