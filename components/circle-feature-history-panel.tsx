  ...
  return <View style={styles.card}>
    ...
    <View style={styles.body}>
      ...
      {selected?.status === "KERNEL_VALIDATED" ? <View style={styles.inspector}>
        ...
        <Text style={styles.label}>STEP GEOMETRY EXPORT</Text><Text style={styles.meta}>{geometryExport.data?.status ?? "AWAITING"} · {geometryExport.data?.format ?? "STEP_GEOMETRY"} · Feature history: {geometryExport.data?.featureHistory ?? "NOT_PRESERVED"}</Text><Text style={styles.meta}>Project {geometryExport.data?.provenance?.projectId ?? "UNKNOWN"} · model {geometryExport.data?.provenance?.model ?? "UNKNOWN"} · revision {geometryExport.data?.provenance?.sourceRevisionId ?? "UNKNOWN"} · branch {geometryExport.data?.provenance?.branch ?? "UNKNOWN"}</Text><Text style={styles.meta}>Timestamp {geometryExport.data?.provenance?.exportedAt ?? "UNKNOWN"} · hash {geometryExport.data?.provenance?.sha256 ?? "UNKNOWN"} · geometry {geometryExport.data?.provenance?.geometryValidation ?? "UNKNOWN"}</Text>{geometryExport.data?.url ? <View style={styles.row}><Pressable style={styles.secondary} onPress={() => void Linking.openURL(geometryExport.data!.url!)}><Text style={styles.secondaryText}>OPEN / DOWNLOAD</Text></Pressable><Pressable style={styles.secondary} onPress={() => void Share.share({ message: `STEP GEOMETRY EXPORT\n${geometryExport.data!.url!}` })}><Text style={styles.secondaryText}>SHARE METADATA</Text></Pressable></View> : null}
