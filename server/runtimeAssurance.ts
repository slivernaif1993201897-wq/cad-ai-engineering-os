...
export async function assessRuntimeAssurance(args: Access): Promise<RuntimeAssuranceAssessment> {
  ...
  for (const definition of gateDefinitions) {
    const dependencyStates = definition.dependencies.map((dependency) => gates.find((gate) => gate.gateId === dependency)?.state ?? "UNKNOWN"); const linked = tests.filter((test) => test.gateId === definition.gateId); const independentPass = linked.filter((test) => test.result === "PASS" && test.evidenceOrigin === "EXTERNAL_OBSERVED" && test.evidenceScope === "INDEPENDENTLY_VERIFIED"); const internal = linked.filter((test) => test.evidenceScope === "INTERNAL_VERIFIED"); let state: RuntimeAssuranceGateState = "UNKNOWN"; let determination: RuntimeAssuranceGateAssessment["determination"] = "EVIDENCE_MISSING";
