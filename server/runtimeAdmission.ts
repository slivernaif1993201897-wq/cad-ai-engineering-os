...
import { listRuntimeAssuranceAssessments, listRuntimeAssuranceEnvironments } from "./runtimeAssurance";
...
export async function evaluateRuntimeAdmission(args: Access & AdmissionInput): Promise<RuntimeAdmissionDecision> {
  ...
  const [jobs, packages, configurations, environments, assessments] = await Promise.all([
    ...
    listRuntimeAssuranceAssessments(args),
  ]);
  ...
  if (!assessment || assessment.gates.some((gate) => gate.state !== "PASS")) { reasonCodes.push("RUNTIME_ASSURANCE_GATES_NOT_PASS"); reasons.push("Runtime assurance gates lack complete independently observed PASS evidence; admission fails closed."); }
