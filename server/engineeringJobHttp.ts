...
export function registerEngineeringJobHttp(app: Express) {
  app.post("/api/projects", async (req, res) => {
    ...
    try {
      ...
      // Do not leave the client waiting when durable persistence is unavailable; empty project creation fails closed.
      return res.status(503).json({ error: "PROJECT_PERSISTENCE_UNAVAILABLE" });
    }
  });
