...
function responseText(action: WorkbenchActionKind, context: CADAgentContext, proposal?: CADChangeProposal, conceptCount = 0): string {
  ...
  if (action === "UPLOAD") return `${selection} Use Attach to associate supported file metadata with this project and conversation. The current workbench validates type and metadata only; it reports unsupported or unparsed content honestly.`;
