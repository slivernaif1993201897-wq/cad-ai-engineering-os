...
def execute_job() -> int:
    ...
    if gmsh.returncode != 0 or (FAILURE_EXERCISE == "GMSH_FAILURE" and not mesh_path.exists()):
        raise RuntimeError(f"GMSH_FAILED_{gmsh.returncode}")
    ...
    raw_solver_result = OUTPUT / "calculix-results.raw.frd"
    solver_result = OUTPUT / "calculix-results.frd"
    raw_solver_result.write_bytes(frd.read_bytes())
    raw_text = raw_solver_result.read_text(encoding="utf-8", errors="replace")
    volatile_time = next((line for line in raw_text.splitlines() if line.strip().startswith("1UTIME")), "")
    solver_result.write_text(raw_text.replace(volatile_time, "    1UTIME              00:00:00                                        "), encoding="utf-8")
    ...
    raw_solver_evidence_path = write_json("raw-solver-evidence.json", {"rawOutputSha256": sha256_file(raw_solver_result), "canonicalOutputSha256": sha256_file(solver_result), "purpose": "retained raw solver audit evidence; excluded from deterministic numerical-result derivation"})
    numerical = {"validationStatus": "PASS" if relative_error <= float(analysis["numericalTolerance"]) else "FAIL", "method": f"Mean loaded-face {analysis['axis']} displacement versus F*L/(E*A)", "referenceDisplacementMm": reference, "solverDisplacementMm": solver_value, "relativeError": relative_error, "tolerance": analysis["numericalTolerance"], "toleranceScope": "INTERNAL_AUTHORIZED_JOB_NUMERICAL_CHECK_ONLY", "frdSha256": sha256_file(solver_result), "inputSha256": input_metadata["calculixInputSha256"]}
