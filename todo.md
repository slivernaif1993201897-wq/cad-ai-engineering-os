...
- [ ] Verify GitHub execution authorization, inspect the bounded benchmark path, and implement or dispatch only a least-privilege workflow supported by actual repository permissions and runner evidence.
- [x] Verify GitHub connection: the canonical repository is accessible with ADMIN viewer permission on `main`; the active bounded benchmark workflow uses `contents: read`, `ubuntu-24.04`, a 15-minute timeout, pinned actions, and artifact retention. No self-hosted runner is registered; secret metadata access is denied with HTTP 403 and secret values were never requested or exposed.
